...
/*******************************************************************************
* Function Name: Reloj_Test_Start
********************************************************************************
*
* Summary:
...
void Reloj_Test_Start(void) 
{
    ...
    Reloj_Test_CLKEN |= Reloj_Test_CLKEN_MASK;
	Reloj_Test_CLKSTBY |= Reloj_Test_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: Reloj_Test_Stop
********************************************************************************
*
* Summary:
...
void Reloj_Test_Stop(void) 
{
    ...
    Reloj_Test_CLKEN &= (uint8)(~Reloj_Test_CLKEN_MASK);
	Reloj_Test_CLKSTBY &= (uint8)(~Reloj_Test_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: Reloj_Test_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((Reloj_Test_CLKEN & Reloj_Test_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = Reloj_Test_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = Reloj_Test_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* Reloj_Test__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(Reloj_Test_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(Reloj_Test_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: Reloj_Test_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: Reloj_Test_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = Reloj_Test_GetSourceRegister();
    uint16 oldDivider = Reloj_Test_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = Reloj_Test_CLKEN & Reloj_Test_CLKEN_MASK;
        ...
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((Reloj_Test_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
/*******************************************************************************
* Function Name: Reloj_Test_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(Reloj_Test_DIV_PTR);
}


/*******************************************************************************
* Function Name: Reloj_Test_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    Reloj_Test_MOD_SRC |= modeBitMask & (uint8)Reloj_Test_MODE_MASK;
}


/*******************************************************************************
* Function Name: Reloj_Test_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    Reloj_Test_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(Reloj_Test_MODE_MASK));
}


/*******************************************************************************
* Function Name: Reloj_Test_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return Reloj_Test_MOD_SRC & (uint8)(Reloj_Test_MODE_MASK);
}


/*******************************************************************************
* Function Name: Reloj_Test_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = Reloj_Test_GetDividerRegister();
    uint8 oldSrc = Reloj_Test_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        Reloj_Test_MOD_SRC |= CYCLK_SSS;
        Reloj_Test_MOD_SRC =
            (Reloj_Test_MOD_SRC & (uint8)(~Reloj_Test_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: Reloj_Test_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return Reloj_Test_MOD_SRC & Reloj_Test_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: Reloj_Test_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    Reloj_Test_PHASE = clkPhase & Reloj_Test_PHASE_MASK;
}


/*******************************************************************************
* Function Name: Reloj_Test_GetPhase
********************************************************************************
*
* Summary:
...
{
    return Reloj_Test_PHASE & Reloj_Test_PHASE_MASK;
}
